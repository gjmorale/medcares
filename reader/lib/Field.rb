...
class Field
	...
	def initialize(text, width = 1, orientation = Setup::Table.header_orientation, date = false)
		if text.is_a? Array
			if text.first.is_a? Array
				@alternatives = text
				@alternatives = @alternatives.map{|s| Multiline.generate s, true}
	...
	def alternatives?
		return (not @alternatives.nil?)
	end

	def set_alternative alt
		@text = alt
		@alternatives = nil
	end
...
class Result < Field
	...
	def clean
		if result != Result::NOT_FOUND and result.is_a? Multiline
			regexs = regex
			regexs = regexs.is_a?(Array) ? regexs : [regexs]
			result.strings.each.with_index do |s, i|
				match = false
				regexs.each do |reg|
					if s.match reg
						match = true
