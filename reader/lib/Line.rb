...
class String
	...
	def match_field field, &block
		if field.alternatives? 
			field.alternatives.each do |alt|
				#puts "#{RegexHelper.regexify alt}\n#{self.to_s}<-"
				self.match(RegexHelper.regexify alt) do |m|
					field.set_alternative alt
					block.call(m)
					return true
				end
			end
		else
			return self.match field.regex, &block
		end
	end
