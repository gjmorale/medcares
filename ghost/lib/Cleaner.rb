class Cleaner
	...
	def execute medcare = '*', plan = '*'
		cache = check medcare, plan
		return cache if cache
		print "Processing"
		files = Dir["#{@input_path}/#{medcare}/#{plan}.pdf"]
		total = files.size
		puts " #{total} files: "
		f_tracker = File.open(@track_path, 'w')
		...
		return "#{@output_path}/" << medcare << "/" << plan
	end

	def check medcare, plan
		return false if medcare == '*' or plan == '*'
		track = File.open(@track_path, 'r')
		track.each_line do |line|
			if line and not line.empty?
				data = line.delete("\n").split(';')
				if medcare == data[0] and plan == data[1]
					return "#{@output_path}/" << medcare << "/" << plan
				end
			end
		end
